    ...
    if pre:
        ...
        for t in tqdm(test_list):
            ...
            if method_name == 'Interformer':
                ...
                sdf_f = f'baseline_results/diffdock/user_predictions_small/{t}/rank1.sdf'  # diffdock
            elif method_name == 'DeepDock':
                sdf_f = f'baseline_results/deepdock/output/{t}_docked.sdf'
            # statistic
            try:
                print(t, end=',')
                ...
                if not docked_str.strip() or not gt_str.strip():
                    print(f"Empty PDB string for {t}")
                    continue
