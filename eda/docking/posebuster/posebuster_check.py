...
def analyze_check_items(df, check_lists):
    """分析每个检查项的失败情况"""
    print("\n=== 检查项失败分析 ===")
    for check_item in check_lists:
        # 获取失败的PDB列表
        failed_pdbs = df[df[check_item] == False]['pdb'].tolist()
        failed_count = len(failed_pdbs)
        total_count = len(df)
        print(f"\n{check_item}:")
        print(f"失败数量: {failed_count}/{total_count} ({round(failed_count*100./total_count, 2)}%)")
        if failed_count > 0:
            print("失败的PDB列表:")
            # 每行显示5个PDB
            for i in range(0, len(failed_pdbs), 5):
                print(", ".join(failed_pdbs[i:i+5]))
    print("\n" + "="*50)
...
if __name__ == '__main__':
    ...
    ###
    # Posebuster
    base = '/home/wlx/桌面/final_result'
    folder = ['energy_posebuster']  # energy_posebuster
    ...
    for f in folder:
        ...
        df.to_csv('/home/wlx/桌面/Interformer_e3_moba_loss/Interformer_e3_moba_loss/tmp/posebuster_check.csv', index=False)
        ###
        # Basic Info
        df = pd.read_csv('/home/wlx/桌面/Interformer_e3_moba_loss/Interformer_e3_moba_loss/tmp/posebuster_check.csv')
        df['pdb'] = df['pdb'].str.upper()
        
        # 分析每个检查项的失败情况
        analyze_check_items(df, check_lists)
        
        # 原有的评估
        eval(df, check_lists)
        v2_pdbs = [x[:4].upper() for x in open('/home/wlx/桌面/Interformer_e3_moba_loss/Interformer_e3_moba_loss/interformer/train/diffdock_splits/posebusters_pdb_ccd_ids.txt').readlines()]
