...
def calc_chunks(cu_seqlen_tuple, moba_chunk_size, device):
    """calc chunks that needs moba attention"""

    cu_seqlen = torch.tensor(cu_seqlen_tuple, dtype=torch.int32, device=device)
    ...
    chunk_sizes = torch.full(
        (num_chunk + 1,), moba_chunk_size, dtype=torch.int32, device=device
    )
    ...
    chunk_to_batch = torch.zeros(
        (num_chunk,), dtype=torch.int32, device=device
    )
    ...
    chunk_to_remain = torch.ones(
        (num_chunk,), dtype=torch.bool, device=device
    )
...
class MixedAttention(torch.autograd.Function):
    ...
    def backward(ctx, d_output):
        ...
        d_moba_output = (
            d_output.view(-1, q.shape[-1]).index_select(0, moba_q_sh_indices).unsqueeze(1)
        )
        moba_output = (
            output.view(-1, q.shape[-1]).index_select(0, moba_q_sh_indices).unsqueeze(1)
        )
...
def moba_attn_varlen(
    ...
    Args:
        ...
        moba_chunk_size (int): the chunk size for moba attention
        moba_topk (int): the topk chunks to be selected for moba attention
    ...
    """ prepare chunk meta """
    cu_seqlen_tuple = tuple(cu_seqlens.cpu().numpy().tolist())
    (
        cu_chunk,
        filtered_chunk_indices,
        num_filtered_chunk,
        filtered_chunk_indices,
        chunk_to_batch,
    ) = calc_chunks(cu_seqlen_tuple, moba_chunk_size, q.device)
    ...
    # 在调用MixedAttention.apply前增加检查
    if moba_cu_seqlen_q[-1] == 0 or moba_cu_seqlen_kv[-1] == 0:
        # 没有有效的MOBA chunk，直接返回自注意力
        return flash_attn_varlen_func(
            q, k, v, cu_seqlens, cu_seqlens, max_seqlen, max_seqlen, causal=True
        )
