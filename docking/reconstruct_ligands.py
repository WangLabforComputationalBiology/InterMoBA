...
def reconstruct_1_ligand_given_paths(
    ...
    with shelve.open(abspath_pkl_normalscore) as db:
        ...
        for index in indices:
            ...
            # --- Start: Added code for logging ---
            log_data = [
                str_pdb_id,
                index, # Log the pose index as well
                pocket_len,
                len_ligand,
                vdw_pair_shape_0
            ]
            header = ["PDB_ID", "Pose_Index", "pocket_len", "len_ligand", "vdw_pair_shape_0"]
            ...
            try:
                ...
                assert pocket_len + len_ligand == vdw_pair_shape_0, \
                       f"Assertion failed for PDB_ID {str_pdb_id}, Pose {index}: {pocket_len} + {len_ligand} != {vdw_pair_shape_0}"

                # Log successful case
                success_file = "successful_cases.csv"
                file_exists = os.path.exists(success_file)
                with open(success_file, 'a', newline='') as f:
                    writer = csv.writer(f)
                    if not file_exists or os.stat(success_file).st_size == 0:
                        writer.writerow(header)
                    writer.writerow(log_data)

            except AssertionError as e:
                # Log unsuccessful case
                error_file = "unsuccessful_cases.csv"
                file_exists = os.path.exists(error_file)
                with open(error_file, 'a', newline='') as f:
                    writer = csv.writer(f)
                    if not file_exists or os.stat(error_file).st_size == 0:
                        writer.writerow(header + ["Error_Message"])
                    writer.writerow(log_data + [str(e)]) # Add error message

                LOGGER.error(f"Assertion failed for {str_pdb_id}, Pose {index}: {e}")
                # Depending on desired behavior, you might want to continue
                # or re-raise the exception. Re-raising will stop processing
                # for this file, which is often desired for debugging.
                raise e # Re-raise the exception

            # --- End: Added code for logging ---
