...
from intermoba.model.ppi_model.ppi_rescorer import PPIScorer
from intermoba.model.transformer.graphormer.intermoba import intermoba
from intermoba.utils.configure import get_exp_configure


def get_model_arg(parser, model=''):
    model_name = model if model else parser.parse_known_args()[0].model
    if model_name == 'intermoba':
        parser = intermoba.add_model_specific_args(parser)
...
def load_from_checkpoint(args, checkpoint):
    ...
    #
    if model_name == 'intermoba':
        model = intermoba.load_from_checkpoint(checkpoint, weights_only=False)
    elif model_name == 'PPIScorer':
