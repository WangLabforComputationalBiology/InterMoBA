...
from intermoba.model.sbdd_model import SBDD
from intermoba.model.transformer.graphormer.graphformer_utils import init_embedding, ComplexEncoder, EncoderLayer, \
...
import shelve
from .MoBA.moba_intermoba_adapter import MoBAintermobaAdapter
...
class VinaScoreHead(nn.Module):
    def __init__(self, hidden_dim, node_featurizer, dropout=0.2, energy_output_folder='', edge_output_layer=True):
        ...
        print(f"# [intermoba] energy_output_fodler:{energy_output_folder}")
        # Ablation Study
        ...
        try:
            if self.energy_output_folder:
                ...
                with shelve.open(output_f) as db:
                    ...
                    print(f"[intermoba] Gussian Score ->{target}-{last_id}")
...
class intermoba(SBDD):
    def __init__(
        ...
        # multi-steps
        self.decoder_step = 1
        self.moba_adapter = MoBAintermobaAdapter(
    ...
    def task_layer(self, output_node, output_edge, batched_data):
        ...
        if bool(torch.isnan(torch.sum(affinity))):
            print("# [intermoba-forward] There is Nan output tensor.")
        ...
        if self.pose_sel_mode:
            ...
            if torch.isnan(gscore_loss):
                print("# [intermoba-forward] G-Score loss is NaN, using default value")
                gscore_loss = torch.tensor(1.0, device=gscore_loss.device)
            if torch.isnan(gscore_pos_loss):
                print("# [intermoba-forward] G-Score pos loss is NaN, using default value")
            ...
            if torch.isnan(atom_loss):
                print("# [intermoba-forward] Atom loss is NaN, using default value")
    ...
    def add_model_specific_args(args):
        parser = args.add_argument_group("intermoba")
        # dim
...
if __name__ == '__main__':
    ...
    args['inference'] = True
    # dataset
    args['work_path'] = '/opt/home/revoli/data_worker/intermoba/poses'
    args['data_path'] = '/opt/home/revoli/data_worker/intermoba/train/100.csv'
