...
class MoBAintermobaAdapter(nn.Module):
    def __init__(self, 
                 hidden_dim: int, 
                 num_heads: int, 
    ...
    def forward(self, 
        ...
        if torch.isnan(node_feats).any() or torch.isnan(edge_feats).any():
            print("# [MoBAintermobaAdapter] Input contains NaN values")
        ...
        if torch.isnan(q).any() or torch.isnan(k).any() or torch.isnan(v).any():
            print("# [MoBAintermobaAdapter] Projection contains NaN values")
        ...
        if torch.isnan(processed_node_feats).any():
            print("# [MoBAintermobaAdapter] MOBA output contains NaN values")
        ...
        if torch.isnan(processed_feats).any():
            print("# [MoBAintermobaAdapter] Final output contains NaN values")
