...
from intermoba.model.model_utils import PolynomialDecayLR
from torchmetrics import PearsonCorrCoef, SpearmanCorrCoef, AUROC
from intermoba.model.loss_utils import get_reg_loss_fn, get_pose_sel_loss_fn
from intermoba.model.mixup import mixup_losses
...
class SBDD(pl.LightningModule):
    ...
    def merge_losses(self, y_hat, y_gt):
        ...
        # 新增：空间约束loss
        if hasattr(self, 'VinaScoreHead'):
            collision_loss = self.VinaScoreHead.collision_loss
            min_dist_loss = self.VinaScoreHead.min_dist_loss
            all_losses.append(0.1 * collision_loss + 0.1 * min_dist_loss)
        loss = sum(all_losses)
        return loss
